...
FILE * OpenFile(const char filename[])
{
    ...
    //  (this will effectivelly limit the string to characters before the '/' being
    //  replaced) checking if the dir pointed by de resulting string exists,
    //  creating if it doesn't exist, restoring the modified '/' (and moving to the
    //  next occurence of '/'. The loop will continue ultil the pointer poinst to
    //  the final null character, whose value can be obtained by deference and is
    //  zero, ending the loop.
    char *pointer = NULL;
